...
function opponent(name, health, maxHealth, pointsId, healthbarId, hardAttackDamage, hardAttackDice, weakAttackDamage, weakAttackDice) {
    ...
    this.attack = async function () {
        ...
        if (x >= hardAttackDice) {
            ...
            let damageText = "⚔️ You take " + hardAttackDamage + " points of critical damage";
...
function healthBarUpdateA(objectHealth, objectHealthMax, pointsClass, healthBar) {
    let x = (objectHealth / objectHealthMax) * 100;
    playerPercent = x.toFixed() + "%";
}

function healthBarUpdateB(objectHealth, objectHealthMax, pointsClass, healthBar) {
...
function resetHealth() {
    ...
    healthBarUpdateB(goblin.health, goblin.maxHealth, goblin.pointsId, goblin.healthbarId);
...
<div>
	<header class="row row-cols-1 row-cols-md-2">
		 <GameCard blank>
			  <heade class="d-flex align-items-center">
			  ...
			  <footer class="py-1">
					<div class="progress">
						 <div class="progress-bar" role="progressbar" style="width: {playerPercent}" min="0" max="100">
							  {playerPercent}
						 </div>
					</div>
			  </footer>
		 ...
		 <GameCard blank>
			  ...
			  <footer class="py-1">
					<div class="progress">
						 <div class="progress-bar" role="progressbar" style="width: {enemyPercent}" min="0" max="100">
							  {enemyPercent}
						 </div>
					</div>
			  </footer>
	...
	<footer class="py-4 text-center">
		 <div class="btn-group btn-group-lg" role="group">
			  ...
			  <button on:click="{func}" type="button" class="btn btn-outline-secondary" disabled="{locked}">{label}</button>
			  {/each}
		 </div>
	</footer>
</div>
