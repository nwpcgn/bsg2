...
$(document).ready(function () {
 ...
 function updateValues() {
   ...
   document.getElementById("lumberjackAmount").innerHTML =
     worker.lumberjack.amount;
   document.getElementById("lumberjackCost").innerHTML =
   ...
   document.getElementById("improvedhouseAmount").innerHTML =
     improvedhouse.amount;
   document.getElementById("improvedhouseCostWood").innerHTML =
     improvedhouse.cost.wood;
   document.getElementById("improvedhouseCostStone").innerHTML =
     improvedhouse.cost.stone;
   document.getElementById("improvedhouseCostIron").innerHTML =
     improvedhouse.cost.iron;
   document.getElementById("improvedhouseResidents").innerHTML =
     improvedhouse.residents;
   ...
   document.getElementById("woodStorageCostWood").innerHTML =
     wood.storageCost.wood;
   document.getElementById("woodStorageCostStone").innerHTML =
     wood.storageCost.stone;
   document.getElementById("stoneStorageAmount").innerHTML = stone.storage;
   document.getElementById("stoneStorageCostWood").innerHTML =
     stone.storageCost.wood;
   document.getElementById("stoneStorageCostStone").innerHTML =
     stone.storageCost.stone;
   document.getElementById("ironStorageAmount").innerHTML = iron.storage;
   document.getElementById("ironStorageCostWood").innerHTML =
     iron.storageCost.wood;
   document.getElementById("ironStorageCostStone").innerHTML =
     iron.storageCost.stone;
   document.getElementById("foodStorageAmount").innerHTML = food.storage;
   document.getElementById("foodStorageCostWood").innerHTML =
     food.storageCost.wood;
   document.getElementById("foodStorageCostStone").innerHTML =
     food.storageCost.stone;

   document.getElementById("autosave").innerHTML =
     "Auto-Saving in " + timer + " Last Save: " + lastSave;
 }
 ...
 $("#createLumberjack").click(function () {
   if (worker.amount < maxPop) {
     ...
     $("#info").prepend(
       $("<p>You need to build more accommodation.</p>").fadeIn("slow")
     );
   }
 });
 ...
 function save_game() {
   ...
   localStorage["rpg_save[improvedhouse]"] = btoa(
     JSON.stringify(improvedhouse)
   );
 ...
 function load_game() {
   ...
   var improvedhouse_save = JSON.parse(
     atob(localStorage["rpg_save[improvedhouse]"])
   );
   ...
   var DoubleSleepingBags_save = localStorage.getItem(
     "rpg_save[DoubleSleepingBags]"
   );
   var TripleSleepingBags_save = localStorage.getItem(
     "rpg_save[TripleSleepingBags]"
   );
   ...
   var MatesRatesStone_save = localStorage.getItem(
     "rpg_save[MatesRatesStone]"
   );
 ...
 $("#buildHouse").click(function () {
   if (wood.amount >= house.cost.wood && stone.amount >= house.cost.stone) {
     ...
     $("#info").prepend(
       $("<p>You need more building materials.</p>").fadeIn("slow")
     );
   }
 });
 ...
 $("#researchHostel").click(function () {
   if (wood.amount >= 400 && stone.amount >= 150) {
     ...
     var getPercent =
       $(".progress-wrap-hostel").data("progress-percent-hostel") / 100;
 ...
 $("#upgradeTwoFingers").click(function () {
   if (wood.amount >= 100 && stone.amount >= 100 && food.amount >= 100) {
     ...
     $("#upgrades").prepend(
       $("<p>Two Fingers | Two Resources Per Click</p>").fadeIn("slow")
     );
     updateValues();
   } else {
 ...
 $("#upgradeFiveFingers").click(function () {
   if (wood.amount >= 450 && stone.amount >= 450 && food.amount >= 120) {
     ...
     $("#upgrades").prepend(
       $("<p>Five Fingers | Five Resources Per Click</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeTenFingers").click(function () {
   if (wood.amount >= 750 && stone.amount >= 750 && food.amount >= 250) {
     ...
     $("#upgrades").prepend(
       $("<p>Ten Fingers | Ten Resources Per Click</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeDoubleSleepingBags").click(function () {
   if (wood.amount >= 100 && stone.amount >= 100 && food.amount >= 100) {
     ...
     $("#upgrades").prepend(
       $("<p>Double Sleeping Bags | Two People, One Tent</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeTripleSleepingBags").click(function () {
   if (wood.amount >= 300 && stone.amount >= 300 && food.amount >= 200) {
     ...
     $("#upgrades").prepend(
       $("<p>Triple Sleeping Bags | Three People, One Tent</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources</p>").fadeIn("slow"));
 ...
 $("#upgradeBunkBeds").click(function () {
   if (wood.amount >= 100 && stone.amount >= 100 && food.amount >= 100) {
     ...
     $("#upgrades").prepend(
       $("<p>Bunk Beds | Five People, One House</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeSharpenAxes").click(function () {
   if (wood.amount >= 50 && stone.amount >= 100 && food.amount >= 50) {
     ...
     $("#upgrades").prepend(
       $("<p>Sharpen Axes | Lumberjacks Chop Two Wood Each</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeSharpenPicks").click(function () {
   if (wood.amount >= 50 && stone.amount >= 100 && food.amount >= 50) {
     ...
     $("#upgrades").prepend(
       $("<p>Sharpen Picks | Miners Mine Two Stone Each</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeSharpenArrows").click(function () {
   if (wood.amount >= 50 && stone.amount >= 100 && food.amount >= 50) {
     ...
     $("#upgrades").prepend(
       $("<p>Sharpen Arrows | Hunters Gather Two Food Each</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
 ...
 $("#upgradeMatesRatesWood").click(function () {
   if (stone.amount >= 150 && food.amount >= 50) {
     ...
     $("#upgrades").prepend(
       $("<p>Mates Rates - Wood | Houses and Tents Cost Less Wood</p>").fadeIn(
         "slow"
       )
     );
     updateValues();
 ...
 $("#upgradeMatesRatesStone").click(function () {
   if (wood.amount >= 150 && food.amount >= 50) {
     ...
     $("#upgrades").prepend(
       $("<p>Mates Rates - Stone | Houses Cost Less Stone</p>").fadeIn("slow")
     );
     updateValues();
   } else {
     $("#info").prepend($("<p>You need more resources.</p>").fadeIn("slow"));
